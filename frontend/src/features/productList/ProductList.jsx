    ...
    <>
      <div className="flex tablet:items-center tablet:justify-between flex-col-reverse tablet:flex-row mb-5 gap-4">
        <div className="flex items-center overflow-x-scroll categories-filter">
          <button
            className={`whitespace-nowrap border-2 bg-gray-100 text-sm text-left w-auto px-3 py-1.5 rounded-full text-gray-700 font-medium mr-2 transition duration-100 ease-in-out ${selectedCategory === 'all'
          ...
          {categories?.map((category) => (
            <button
              className={`capitalize whitespace-nowrap border-2 bg-gray-100 text-sm text-left w-auto px-3 py-1.5 rounded-full text-gray-700 font-medium mr-2 transition duration-300 ease-in-out ${selectedCategory === category
                  ? 'border-gray-700 hover:bg-gray-200'
                  : 'border-gray-100 hover:bg-gray-200 hover:border-gray-200'
                } disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-400 disabled:border-transparent`}
              key={category}
              ...
              filteredProducts?.map((product) => (
                ...
                >
                  <div className="mb-2">
                    ...
                    <h1 className="mb-0.5 mobile:mb-0.5 text-xs font-bold text-gray-700 line-clamp-2 hover:line-clamp-none">
                      {product?.title}
                    </h1>
                    <span className="block capitalize text-xs text-gray-500 mb-2.5 mobile:mb-2.5">
                      {product?.author}
                    </span>
                  ...
                  <div className="absolute top-3 right-3">
                    <HeartIcon
                      size={22}
                      className={`cursor-pointer ${isWishlistProduct(product)
